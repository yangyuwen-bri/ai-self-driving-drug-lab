...
def compute_desirability(
    ...
    for objective in objectives:
        ...
        if objective.mode == "match_bell":
            score = bell_score(values[name], target[name], objective.tolerance or 0.5)
            ...
            score = normalized_ramp(values[name], lower, upper)
        else:
...
def sample_parameters(scenario: ScenarioSpec, rng: random.Random) -> dict[str, float]:
    return {spec.name: round(rng.uniform(spec.lower, spec.upper), 4) for spec in scenario.parameters}
